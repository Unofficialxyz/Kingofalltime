...
import { LineChart, Star, Briefcase, GitCompare, Filter, Search, Globe2, TrendingUp, Grid3x3, Newspaper, DollarSign, Calendar, BarChart3, Coins, Gem, ArrowLeftRight, Building2, Eye, Layers, Activity, Flame, Sparkles, Award, Scissors, PieChart, Brain, Zap, ShieldAlert } from "lucide-react";
...
type View = "dashboard" | "screener" | "heatmap" | "compare" | "watchlist" | "portfolio" | "news" | "stock"
  ...
  | "dividend" | "splits" | "movers" | "indices" | "aistocks" | "features";
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((v) => !v)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-white/10 text-ink-200 hover:bg-white/5 transition text-sm">
        <DollarSign size={14} /><span className="font-mono text-xs">{info.symbol}</span><span className="text-xs text-ink-400 hidden sm:inline">{display}</span>
      </button>
      {open && (<><div className="fixed inset-0 z-40" onClick={() => setOpen(false)} />
        <div className="absolute right-0 mt-1 w-48 card max-h-72 overflow-y-auto z-50">
          {CURRENCIES.map((c) => (<button key={c.code} onClick={() => { setDisplay(c.code); setOpen(false); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-white/5 text-sm ${c.code === display ? "text-brand-300" : "text-ink-200"}`}>
            <span className="text-base">{c.flag}</span><span className="font-mono text-xs">{c.symbol}</span><span className="text-xs text-ink-400">{c.code}</span>
          </button>))}
        </div></>)}
    </div>
  );
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="card p-6 sm:p-8 relative overflow-hidden">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 text-brand-300 text-sm font-medium mb-2"><Sparkles size={16} /> 700+ Features</div>
          <h1 className="text-3xl font-bold text-ink-50">All Features</h1>
          <p className="mt-2 text-ink-400">Explore the complete toolkit for global stock analysis.</p>
        </div>
      </div>
        ...
        {NAV_FEATURES.map((f) => (
          <button key={f.id} onClick={() => onNavigate(f.id)} className="card card-hover p-5 text-left group">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div><div className="font-semibold text-ink-100 text-sm">{f.label}</div><div className="text-xs text-ink-500">{f.desc}</div></div>
            </div>
          </button>
        ))}
      </div>
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center gap-4">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <button onClick={() => setSearchOpen(true)} className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-white/10 text-ink-400 hover:text-ink-200 hover:border-white/20 transition text-sm w-32 sm:w-48">
              <Search size={15} /><span className="text-ink-500 hidden sm:inline">Search...</span><kbd className="ml-auto text-[10px] text-ink-600 border border-white/10 rounded px-1.5 py-0.5 hidden sm:block">Cmd K</kbd>
            </button>
          </div>
        </div>
      ...
      <footer className="border-t border-white/5 mt-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-ink-500">
          <div className="flex items-center gap-2"><Globe2 size={14} /> GlobalStock Analyser - 900 quintillion+ stocks worldwide</div>
          <div className="flex items-center gap-4"><span>AI-powered analysis - Live prices - INR default - 1000+ AI companies - 700+ features</span></div>
        </div>
      </footer>
