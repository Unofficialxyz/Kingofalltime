...
export function SearchPalette({ open, onClose, onSelect }: Props) {
  ...
  const results = useMemo(() => {
    if (!q) return STOCK_UNIVERSE.slice(0, 10);
    const lower = q.toLowerCase();
    return STOCK_UNIVERSE.filter((s) => s.symbol.toLowerCase().includes(lower) || s.name.toLowerCase().includes(lower)).slice(0, 20);
  }, [q]);
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[10vh] px-4" onClick={onClose}>
      ...
      <div className="relative w-full max-w-lg card p-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search 30,000+ stocks…" className="flex-1 bg-transparent text-ink-100 text-sm focus:outline-none" />
