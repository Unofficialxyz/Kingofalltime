...
function generateAIStocks(): StockMeta[] {
  ...
  for (let i = 0; i < 520; i++) {
    const rd = REGIONS_DATA[i % REGIONS_DATA.length];
    ...
    const symbol = `AI${p1}${p2}${p3}${suffix}`.slice(0, 7);
    const name = `${p1}${p2} AI ${pick(AI_INDUSTRIES, i)}`;
    stocks.push({
      symbol, name, exchange: rd.exchange, region: rd.region, currency: rd.currency,
      sector: i % 5 === 0 ? 'Healthcare' : 'Technology', industry: pick(AI_INDUSTRIES, i),
    });
  }
  return stocks;
}
    ...
    for (const prefix of rd.prefixes) {
      for (const root of rd.nameRoots) {
        for (let v = 0; v < 8; v++) {
          ...
          stocks.push({
            symbol, name: `${prefix}${root}${suffix} ${rd.region === 'United States' ? 'Inc' : rd.region === 'India' ? 'Ltd' : 'Corp'}`,
            exchange: rd.exchange, region: rd.region, currency: rd.currency,
            sector: SECTORS_LIST[Math.floor(seeded(idx) * SECTORS_LIST.length)], industry: root,
          });
...
for (const s of [...aiStocks, ...generalStocks]) {
  if (!seen.has(s.symbol)) { seen.add(s.symbol); merged.push(s); }
}
